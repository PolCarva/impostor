...
const ImpostorIcon = ({
  ...
  const randomClass = randomRotate
    ? `doodle-icon-random-${randomIndex + 1}`
    : "";
...
const GAME_MODES: Record<GameMode, GameModeConfig> = {
  classic: {
    name: "Clásico",
    description:
      "El modo original: un impostor debe descubrir la palabra secreta",
    icon: UserSearch,
    minPlayers: 3,
  },
  lost: {
  ...
  chaos: {
    name: "Locura",
    description:
      "Con probabilidad aleatoria, todos pueden ser impostores, nadie será impostor pero todos tendrán palabras diferentes, o nadie será impostor pero todos compartirán la misma palabra",
    icon: Zap,
    minPlayers: 3,
  },
  random: {
    name: "Aleatorio",
    description:
      "Elige aleatoriamente entre todos los modos de juego disponibles. ¡Nadie sabrá cuál está activo!",
    icon: Shuffle,
    minPlayers: 3,
  },
};
...
export default function ImpostorGame() {
  ...
  const [chaosPlayerWords, setChaosPlayerWords] = useState<
  ...
  useEffect(() => {
    localStorage.setItem(
      "impostor-help-enabled",
      JSON.stringify(impostorHelpEnabled)
    );
  }, [impostorHelpEnabled]);
  ...
  useEffect(() => {
    if (players.length > 0) {
      const hasSpecialRole =
        selectedGameMode === "lost" || selectedGameMode === "jester";
      const newMaxImpostors = Math.max(
        1,
        players.length - (hasSpecialRole ? 3 : 2)
      );
  ...
  const getAvailableWords = () => {
    ...
    const selectedWords = selectedCategories.flatMap((category) => {
      if (DEFAULT_CATEGORIES[category as keyof typeof DEFAULT_CATEGORIES]) {
        return (
          DEFAULT_CATEGORIES[category as keyof typeof DEFAULT_CATEGORIES] || []
        );
      }
  ...
  const addPlayer = () => {
    if (playerMode === "manual") {
      ...
      const count =
        typeof quickPlayerCount === "string"
          ? parseInt(quickPlayerCount)
          : quickPlayerCount;
      if (count > 0) {
  ...
  const startGame = () => {
    ...
    if (
      DEFAULT_CATEGORIES[
        selectedCategoryName as keyof typeof DEFAULT_CATEGORIES
      ]
    ) {
      categoryWords =
        DEFAULT_CATEGORIES[
    ...
    const shuffledIndices = shuffleArrayWithSeed(
    ...
    const randomFirstPlayer = shuffleArrayWithSeed(
      [...Array(players.length).keys()],
      firstPlayerSeed
    )[0];
    ...
    if (actualGameMode === "chaos") {
      ...
      const chaosRoll = Math.floor(
        (Math.sin(chaosSeed) * 10000 -
          Math.floor(Math.sin(chaosSeed) * 10000)) *
          10
      ); // 0-9 usando semilla

      // Probabilidad 50% si venimos de modo Aleatorio, sino 1/10 normal
      const forceSpecialScenario =
        selectedGameMode === "random" &&
        Math.sin(chaosSeed + 1000) * 10000 -
          Math.floor(Math.sin(chaosSeed + 1000) * 10000) <
          0.5;
      if (forceSpecialScenario || chaosRoll === 0) {
        ...
        const chaosType = Math.floor(
          (Math.sin(chaosTypeSeed) * 10000 -
            Math.floor(Math.sin(chaosTypeSeed) * 10000)) *
            3
        ); // 0, 1 o 2 usando semilla
        ...
        if (chaosType === 0) {
          ...
          while (extendedWordPool.length < players.length) {
            const additionalWords = getAvailableWords().filter(
              (word) => !extendedWordPool.includes(word)
            );
          ...
          for (let i = 0; i < players.length; i++) {
            ...
            let availableWordsForPlayer = extendedWordPool.filter(
            ...
            const playerWordSeed = generateGameSeed(
              players.length,
              Date.now() + 7 + i
            );
            const shuffledAvailableWords = shuffleArrayWithSeed(
              availableWordsForPlayer,
              playerWordSeed
            );
        ...
        const chaosFirstPlayerSeed = generateGameSeed(
          players.length,
          Date.now() + 8
        );
        const chaosFirstPlayer = shuffleArrayWithSeed(
          [...Array(players.length).keys()],
          chaosFirstPlayerSeed
        )[0];
        ...
        // Track game start
        const hasCustomCategory = categoriesToUse.some(
          (cat) => cat in customCategories
        );
        trackGameStart({
    ...
    if (actualGameMode === "lost") {
      ...
      const nonImpostorIndices = shuffledIndices.filter(
        (i) => !selectedImpostors.includes(i)
      );
      const shuffledNonImpostors = shuffleArrayWithSeed(
        nonImpostorIndices,
        lostSeed
      );
      ...
      let differentWord;
      const availableDifferentWords = categoryWords.filter(
        (word) => word !== randomWord
      );
      if (availableDifferentWords.length > 0) {
        const shuffledWords = shuffleArrayWithSeed(
          availableDifferentWords,
          wordSeed
        );
        ...
        const fallbackWords = availableWords.filter(
          (word) => word !== randomWord
        );
        const shuffledFallbackWords = shuffleArrayWithSeed(
          fallbackWords,
          wordSeed
        );
  ...
  const updateWordInCategory = (index: number, newWord: string) => {
    if (
      newWord.trim() &&
      !editingCategoryWords.some(
        (word, i) => i !== index && word.trim() === newWord.trim()
      )
    ) {
      setEditingCategoryWords((prev) =>
        prev.map((word, i) => (i === index ? newWord : word))
      );
    }
  };
  ...
  const generateWordsWithAI = async (prompt: string) => {
    ...
    try {
      ...
      console.log(
        `✅ ¡Éxito! Se generaron ${data.words.length} elementos usando ${data.model}`
      );
      showAlert(
        "success",
        "¡Palabras generadas!",
        `Se generaron ${data.words.length} elementos para "${prompt}"`
      );
      ...
      showAlert(
        "error",
        "Error de conexión",
        "No se pudo conectar con el servidor de IA. Se agregaron palabras de ejemplo en su lugar."
      );
          ...
          <CardContent className="p-4 md:p-6 flex flex-col flex-1 min-h-0">
            <div className="flex items-center justify-between mb-4 shrink-0">
              <div className="flex items-center gap-2">
                <DoodleIcon
                  ...
                  size={28}
                  thick
                  className="animate-[bounce-soft_2s_ease-in-out_infinite]"
                  uniqueId="theme-header"
                />
            ...
            <div className="flex-1 overflow-y-auto pr-2 -mr-2 min-h-0 overflow-x-hidden">
              <div className="grid pt-4 grid-cols-1 md:grid-cols-2 gap-3 px-1">
                {(Object.keys(COLOR_PALETTES) as PaletteName[]).map(
                  (paletteName) => {
                      ...
                      >
                        <div className="flex flex-col items-start gap-2">
                          <div className="w-full flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              {palette.icon && (
                                <DoodleIcon
                                  ...
                                  style={{
                                    stroke: palette.colors["card-foreground"],
                                  }}
                                />
                              )}
                              <span
                                ...
                                style={{
                                  color: palette.colors["card-foreground"],
                                }}
                              >
                                {palette.name}
                              </span>
                            ...
                            {isSelected && (
                              <div
                                className="rounded-full p-1 animate-[bounce-soft_1s_ease-in-out_infinite]"
                                style={{
                              ...
                              >
                                <Check
                                  className="h-3 w-3"
                                  style={{
                                    color: palette.colors["primary-foreground"],
                                  }}
                                />
                              </div>
                            )}
                          ...
                          <div className="w-full flex gap-1.5">
                            <div
                              className="h-8 flex-1 rounded-[10px_3px_10px_3px/3px_10px_3px_10px] border-2 border-white/20"
                              style={{
                                backgroundColor: palette.colors.primary,
                              }}
                            />
            ...
            <div className="shrink-0 pt-4 border-t border-border/50 mt-4">
              <Button onClick={closeThemeSettings} className="w-full" size="lg">
                ¡Listo! →
              </Button>
            </div>
          </CardContent>
      ...
      >
        ...
        {customAlert.show && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
            <Card
              className={`w-full max-w-sm border-[3px] ${
                ...
                <div className="bg-muted/30 p-4 rounded-[20px_8px_20px_8px/8px_20px_8px_20px] border-[3px] border-dashed border-border">
                  ...
                  <div className="flex flex-col gap-2">
                    <Textarea
                      ...
                      value={aiPrompt}
                      onChange={(e) =>
                        setAiPrompt(e.target.value.slice(0, 200))
                      }
                ...
                <div>
                    ...
                    {editingCategoryWords.map((word, index) => (
                      ...
                      >
                        <Input
                          ...
                          value={word}
                          onChange={(e) =>
                            updateWordInCategory(index, e.target.value)
                          }
                  ...
                  {editingCategoryWords.length === 0 && (
                    <div className="text-center py-8 text-muted-foreground">
                      ...
                      <p className="text-xs">
                        Usa el asistente mágico o agrega palabras manualmente
                      </p>
                    </div>
                  )}
                </div>
              ...
              <CardContent className="p-4 md:p-6 flex flex-col flex-1 min-h-0 overflow-hidden">
                <div className="flex items-center justify-between mb-4 shrink-0">
                  ...
                  <Button
                    variant="ghost"
                    size="icon"
                    onClick={() => setShowGameModesInfo(false)}
                    aria-label="Cerrar información de modos"
                  >
                    <X className="h-5 w-5" />
                  </Button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto pr-2 -mr-2 min-h-0">
                  <div className="space-y-4 text-left">
                    {(
                      Object.entries(GAME_MODES) as [GameMode, GameModeConfig][]
                    ).map(([modeKey, modeConfig]) => (
                      <div
                        key={modeKey}
                        className="bg-muted/30 p-4 rounded-[20px_8px_20px_8px/8px_20px_8px_20px] border-2 border-border"
                      >
                        <div className="flex items-start gap-3 mb-3">
                          ...
                          <div className="flex-1">
                            <h4 className="text-lg font-title font-bold text-foreground mb-1">
                              {modeConfig.name}
                            </h4>
                            ...
                            <p className="text-xs text-muted-foreground font-medium">
                              Mínimo {modeConfig.minPlayers} jugadores
                            </p>
                          </div>
                        </div>
                            ...
                            <>
                              <p className="flex items-start gap-2">
                                <DoodleIcon
                                  icon={Users}
                                  size={20}
                                  className="stroke-[2.5] mt-0.5 shrink-0"
                                  uniqueId={`classic-how-${modeKey}`}
                                />
                                <span>
                                  <strong>Cómo se juega:</strong> Todos los
                                  jugadores comparten una palabra secreta,
                                  excepto el impostor que debe descubrirla sin
                                  ser descubierto.
                                </span>
                              </p>
                              <p className="flex items-start gap-2">
                                <DoodleIcon
                                  ...
                                  size={20}
                                  className="stroke-[2.5] mt-0.5 shrink-0"
                                  uniqueId={`classic-win-${modeKey}`}
                                />
                                <span>
                                  <strong>Cómo ganar:</strong> Los jugadores
                                  honestos ganan si identifican al impostor. El
                                  impostor gana si adivina la palabra sin ser
                                  descubierto.
                                </span>
                              </p>
                              <p className="flex items-start gap-2">
                                <DoodleIcon
                                  ...
                                  size={20}
                                  className="stroke-[2.5] mt-0.5 shrink-0"
                                  uniqueId={`classic-strategy-${modeKey}`}
                                />
                                <span>
                                  <strong>Estrategia:</strong> Da pistas claras
                                  pero no directas sobre la palabra. Observa
                                  inconsistencias en las pistas de los demás.
                                </span>
                              </p>
                            </>
                          )}
                            ...
                            <>
                              <p className="flex items-start gap-2">
                                <DoodleIcon
                                  ...
                                  size={20}
                                  className="stroke-[2.5] mt-0.5 shrink-0"
                                  uniqueId={`lost-how-${modeKey}`}
                                />
                                <span>
                                  <strong>Cómo se juega:</strong> Un jugador (El
                                  Perdido) tiene una palabra completamente
                                  diferente pero no lo sabe. Los impostores
                                  deben descubrir la palabra común del grupo.
                                </span>
                              </p>
                              <p className="flex items-start gap-2">
                                <DoodleIcon
                                  ...
                                  size={20}
                                  className="stroke-[2.5] mt-0.5 shrink-0"
                                  uniqueId={`lost-win-${modeKey}`}
                                />
                                <span>
                                  <strong>Cómo ganar:</strong> Los jugadores
                                  honestos (incluyendo El Perdido) ganan si
                                  identifican correctamente a los impostores.
                                </span>
                              </p>
                              <p className="flex items-start gap-2">
                                <DoodleIcon
                                  ...
                                  size={20}
                                  className="stroke-[2.5] mt-0.5 shrink-0"
                                  uniqueId={`lost-strategy-${modeKey}`}
                                />
                                <span>
                                  <strong>Estrategia:</strong> El Perdido dará
                                  pistas confusas sin saber por qué. Los demás
                                  deben observar quién parece perdido o da
                                  pistas incoherentes.
                                </span>
                              </p>
                            </>
                            ...
                            <>
                              <p className="flex items-start gap-2">
                                <DoodleIcon
                                  ...
                                  size={20}
                                  className="stroke-[2.5] mt-0.5 shrink-0"
                                  uniqueId={`jester-how-${modeKey}`}
                                />
                                <span>
                                  <strong>Cómo se juega:</strong> Un jugador (El
                                  Bufón) conoce la palabra pero tiene un
                                  objetivo secreto: convencer al grupo de que lo
                                  elimine.
                                </span>
                              </p>
                              <p className="flex items-start gap-2">
                                <DoodleIcon
                                  ...
                                  size={20}
                                  className="stroke-[2.5] mt-0.5 shrink-0"
                                  uniqueId={`jester-win-${modeKey}`}
                                />
                                <span>
                                  <strong>Cómo ganar:</strong> El Bufón gana si
                                  es votado para ser eliminado. Los demás ganan
                                  si identifican correctamente al impostor.
                                </span>
                              </p>
                              <p className="flex items-start gap-2">
                                <DoodleIcon
                                  ...
                                  size={20}
                                  className="stroke-[2.5] mt-0.5 shrink-0"
                                  uniqueId={`jester-strategy-${modeKey}`}
                                />
                                <span>
                                  <strong>Estrategia:</strong> El Bufón debe dar
                                  pistas plausibles pero sospechosas. Los demás
                                  deben identificar quién parece estar jugando
                                  para ser eliminado.
                                </span>
                              </p>
                            </>
                            ...
                            <>
                              <p className="flex items-start gap-2">
                                <DoodleIcon
                                  ...
                                  size={20}
                                  className="stroke-[2.5] mt-0.5 shrink-0"
                                  uniqueId={`chaos-how-${modeKey}`}
                                />
                                <span>
                                  <strong>Cómo se juega:</strong> Con
                                  probabilidad de 1/10, puede activarse uno de
                                  tres escenarios especiales:
                                  <br />
                                  1. Todos los jugadores son impostores
                                  <br />
                                  2. Nadie es impostor pero todos tienen
                                  palabras diferentes
                                  <br />
                                  3. Nadie es impostor pero todos comparten la
                                  misma palabra
                                </span>
                              </p>
                              <p className="flex items-start gap-2">
                                <DoodleIcon
                                  ...
                                  size={20}
                                  className="stroke-[2.5] mt-0.5 shrink-0"
                                  uniqueId={`chaos-win-${modeKey}`}
                                />
                                <span>
                                  <strong>Cómo ganar:</strong> Igual que el modo
                                  clásico, con la particularidad de que cada
                                  tanto TODOS DICEN INCOHERENCIAS Y NADIE
                                  ENTIENDE NADA!
                                </span>
                              </p>
                              <p className="flex items-start gap-2">
                                <DoodleIcon
                                  ...
                                  size={20}
                                  className="stroke-[2.5] mt-0.5 shrink-0"
                                  uniqueId={`chaos-strategy-${modeKey}`}
                                />
                                <span>
                                  <strong>Estrategia:</strong> Mantén la calma y
                                  observa las inconsistencias. En el modo
                                  locura, las dinámicas cambian completamente,
                                  así que adapta tu estrategia según el
                                  escenario que creas que estáactivado.
                                </span>
                              </p>
                            </>
                          )}
                            ...
                            <>
                              <p className="flex items-start gap-2">
                                <DoodleIcon
                                  ...
                                  size={20}
                                  className="stroke-[2.5] mt-0.5 shrink-0"
                                  uniqueId={`random-how-${modeKey}`}
                                />
                                <span>
                                  <strong>Cómo se juega:</strong> El sistema
                                  elige aleatoriamente entre Clásico, Perdido,
                                  Bufón y Locura. Ni el organizador ni los
                                  jugadores saben qué modo está activo hasta que
                                  comienza el juego.
                                </span>
                              </p>
                              <p className="flex items-start gap-2">
                                <DoodleIcon
                                  ...
                                  size={20}
                                  className="stroke-[2.5] mt-0.5 shrink-0"
                                  uniqueId={`random-win-${modeKey}`}
                                />
                                <span>
                                  <strong>Cómo ganar:</strong> Depende del modo
                                  elegido aleatoriamente. ¡Descubre las reglas
                                  mientras juegas!
                                </span>
                              </p>
                              <p className="flex items-start gap-2">
                                <DoodleIcon
                                  ...
                                  size={20}
                                  className="stroke-[2.5] mt-0.5 shrink-0"
                                  uniqueId={`random-strategy-${modeKey}`}
                                />
                                <span>
                                  <strong>Estrategia:</strong> Mantén la mente
                                  abierta y observa atentamente. Cualquier cosa
                                  puede pasar en el modo aleatorio.
                                </span>
                              </p>
                            </>
                          )}
            ...
            <Card className="w-full max-w-2xl max-h-[90vh] flex flex-col">
              <CardContent className="p-4 md:p-6 flex flex-col flex-1 min-h-0 overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto pr-2 -mr-2 min-h-0">
                  <div className="space-y-4 text-left">
                    <div>
                      <h2 className="text-lg font-title font-bold text-foreground mb-2">
                      ...
                      <p className="text-sm text-foreground mb-3">
                        ...
                        identificarlo. Es similar a Among Us pero con palabras,
                        perfecto para grupos de amigos, familia y reuniones.
                      </p>
                    </div>
                    ...
                    <div className="bg-orange-50 dark:bg-orange-950/20 p-4 rounded-[20px_8px_20px_8px/8px_20px_8px_20px] border-2 border-orange-200 dark:border-orange-800/50">
                      ...
                      <p className="text-sm text-muted-foreground mb-3">
                        El Impostor es completamente gratuito. Si quieres apoyar
                        el desarrollo y mantenerlo vivo, ¡tu contribución es
                        bienvenida!
                      </p>
                      ...
                      <div className="space-y-2 mb-3">
                        ...
                        <Button
                          ...
                          onClick={() =>
                            window.open(
                              "https://link.mercadopago.com.uy/pablocarvalhogimenez",
                              "_blank"
                            )
                          }
                        >
                        ...
                        <div className="pt-2 border-t border-border/50">
                          <Button
                            ...
                            onClick={() =>
                              window.open(
                                "https://buymeacoffee.com/pablocarvalho",
                                "_blank"
                              )
                            }
                          >
                    ...
                    <div>
                      ...
                      <p className="text-sm text-foreground mb-3">
                        Cada jugador mira su carta en secreto. Todos excepto el
                        impostor ven la palabra secreta. Por turnos, cada
                        jugador da una pista sobre la palabra sin decirla
                        directamente. Después de las rondas, todos votan para
                        descubrir quién es el impostor. Se necesitan mínimo 3
                        jugadores y es más divertido con grupos de 5 a 10
                        personas.
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <p className="text-sm text-foreground mb-3">
                        ...
                        tus propias categorías personalizadas o usar las
                        predefinidas. Juego de palabras interactivo para grupos
                        de 3 o más jugadores.
                      </p>
                      <p className="text-sm text-foreground">
                        ¿Buscas más juegos del creador? El Impostor es similar a
                        otros juegos de deducción como
                        <a
                          href="https://www.roblox.com/games/6284583030/Among-Us"
                          target="_blank"
                        ...
                        pero con palabras. Si te gusta Wordle, también puedes
                        probar
                        <a
                          href="https://www.wordleinfinito.com"
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-primary font-bold hover:underline mx-1"
                        >
          ...
          <CardContent className="p-4 md:p-6 flex flex-col flex-1 min-h-0">
            <div className="flex justify-between items-center mb-2 shrink-0 gap-2">
              ...
              {!isStandalone && isIOS ? (
                <Button
                  ...
                  size="sm"
                  className="gap-2 bg-primary hover:bg-primary/90 text-primary-foreground shadow-[3px_3px_0_0_var(--border)] border-2 border-foreground/20"
                >
                  <DoodleIcon
                    icon={Download}
                    size={18}
                    className="stroke-[2.5]"
                    uniqueId="download-btn"
                  />
                  Descargar App
                </Button>
              ) : (
            ...
            <div className="text-center mb-4 shrink-0 relative">
              ...
              <div className="flex justify-center mb-2">
                <ImpostorIcon
                  size={48}
                  className="animate-[bounce-soft_2s_ease-in-out_infinite]"
                  uniqueId="categories-title"
                />
              </div>
              <h2 className="text-3xl md:text-4xl font-title font-bold text-primary mb-2">
                El Impostor{" "}
                <span className="sr-only">- Juego de Fiesta Gratis</span>
              </h2>
              ...
              <div className="mb-3">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <div className="flex justify-center gap-1 px-1">
                    {(["classic", "lost", "jester"] as GameMode[]).map(
                      (modeKey) => {
                    ...
                    return (
                      ...
                      >
                        <DoodleIcon
                          icon={CategoryIcon}
                          size={16}
                          className="inline-block mr-1.5 stroke-[2.5]"
                          uniqueId={`category-${category}-${index}`}
                        />
                ...
                <div className="mb-4 overflow-hidden">
                  <h3 className="text-lg font-title font-bold text-foreground mb-2 flex items-center gap-2">
                    <DoodleIcon
                      ...
                      size={20}
                      className="stroke-[2.5]"
                      uniqueId="custom-header"
                    />
                    Tus Categorías
                  </h3>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-2 px-1">
                    {Object.entries(customCategories).map(
                      ([categoryName, words], index) => {
                        const isSelected =
                              ...
                              <div className="flex flex-col">
                                <span className="font-bold text-xs flex items-center gap-1">
                                  <DoodleIcon
                                    ...
                                    size={12}
                                    className="stroke-[2.5]"
                                    uniqueId={`custom-cat-${categoryName}-${index}`}
                                  />
                                  {categoryName}
                                </span>
                                <span
            ...
            <div className="shrink-0 pt-4 border-t border-border/50">
              <div className="text-center mb-4">
                <p
                  className={`text-xs py-1.5 px-3 rounded-full flex items-center gap-2 justify-center ${
                    selectedCategories.length === 0
                      ? "bg-muted/50"
                      : "bg-primary/20"
                  }`}
      ...
      >
        ...
        <Card className="w-full max-w-lg relative z-10 flex flex-col max-h-[90vh]">
          <CardContent className="p-4 md:p-6 flex flex-col flex-1 min-h-0">
            ...
            <div className="text-center mb-4 shrink-0">
              <div className="flex justify-center mb-2">
                <ImpostorIcon
                  ...
                  uniqueId="setup-title"
                />
              </div>
              <h2 className="text-3xl md:text-4xl font-title font-bold text-primary mb-1">
                El Impostor
              </h2>
              <p className="text-muted-foreground text-sm md:text-base flex items-center justify-center gap-2">
            ...
            <div className="flex-1 overflow-y-auto overflow-x-hidden pr-2 -mr-2 min-h-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-muted-foreground">
                          {quickPlayerCount === 1 || quickPlayerCount === "1"
                            ? "jugador"
                            : "jugadores"}
                        </span>
                      </div>
              ...
              <div className="mb-4 p-3 bg-primary/10 rounded-[20px_8px_20px_8px/8px_20px_8px_20px] border-[3px] border-primary/30">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <DoodleIcon
                      icon={GAME_MODES[selectedGameMode].icon}
                      size={20}
                      thick
                      className="stroke-[2.5]"
                      uniqueId="setup-mode-icon"
                    />
            ...
            {/* Footer fijo con CTA */}
            <div className="shrink-0 pt-4 border-t border-border/50">
              {players.length < GAME_MODES[selectedGameMode].minPlayers &&
                players.length > 0 && (
                  <p className="text-center text-xs text-destructive mb-3 animate-[shake_0.5s_ease-in-out] flex items-center justify-center gap-1">
                    Necesitas al menos {GAME_MODES[selectedGameMode].minPlayers}{" "}
                    jugadores para este modo
                  </p>
                )}
              <Button
                onClick={startGame}
    ...
    return (
      ...
      >
        ...
        <div className="w-full max-w-md flex flex-col items-center relative z-10">
          <div className="text-center mb-8">
            <div className="inline-flex items-center gap-2 bg-muted px-5 py-2 rounded-full border-2 border-border mb-4">
              <DoodleIcon
                icon={Users}
                size={20}
                className="stroke-[2.5] text-muted-foreground"
                uniqueId={`player-indicator-${currentPlayer}`}
              />
          ...
          <div
            className={`flip-card w-full max-w-xs sm:max-w-sm aspect-[2.5/3.5] mb-8 ${
              isFlipped ? "flipped" : ""
            }`}
          ...
          >
            <div className="flip-card-inner w-full h-full">
              {/* Front of card */}
              <div
                className={`flip-card-front absolute w-full h-full rounded-[20px] border-4 border-foreground/80 bg-card shadow-[8px_8px_0_0_var(--primary)] cursor-pointer hover:shadow-[10px_10px_0_0_var(--primary)] hover:translate-x-[-2px] hover:translate-y-[-2px] transition-all select-none overflow-hidden ${
              ...
              >
                ...
                <div className="h-full flex flex-col items-center justify-center p-8 relative">
                  ...
                  <h2 className="text-3xl md:text-4xl font-title font-bold text-primary mb-4 text-center">
                ...
                <span className="absolute bottom-3 left-3 text-2xl opacity-50">
                  ✦
                </span>
                <span className="absolute bottom-3 right-3 text-2xl opacity-50">
                  ✦
                </span>
              </div>
                        ...
                        <>
                          <div className="mb-4 animate-[shake_0.3s_ease-in-out_infinite]">
                            <DoodleIcon
                              ...
                              size={72}
                              thick
                              uniqueId="card-back-impostor"
                            />
                          </div>
                          <h3 className="text-4xl font-title font-bold text-destructive mb-4">
                            ¡IMPOSTOR!
                          </h3>
                        ...
                        <>
                          <div className="mb-4 animate-[twinkle_2s_ease-in-out_infinite]">
                            <DoodleIcon
                              ...
                              size={72}
                              thick
                              uniqueId="card-back-lost"
                            />
                          </div>
                          <h3 className="text-3xl md:text-4xl font-title font-bold text-secondary mb-4 text-center">
                            {playerWord}
                          </h3>
                          <p className="text-muted-foreground text-center text-sm flex items-center justify-center gap-1">
                        ...
                        <>
                          <div className="mb-4 animate-[twinkle_2s_ease-in-out_infinite]">
                            <DoodleIcon
                              ...
                              size={72}
                              thick
                              uniqueId="card-back-jester"
                            />
                          </div>
                          <h3 className="text-4xl font-title font-bold text-purple-500 mb-2 text-center">
                            ¡ERES EL BUFÓN!
                          </h3>
                          <h4 className="text-3xl md:text-4xl font-title font-bold text-secondary mb-4 text-center">
                            {selectedWord}
                          </h4>
        ...
        <CardContent className="p-8 text-center">
          <div className="flex justify-center mb-6 animate-[bounce-soft_1s_ease-in-out_infinite]">
            <DoodleIcon
              ...
              size={72}
              thick
              uniqueId="finished-icon"
            />
          </div>
          <h2 className="text-4xl font-title font-bold text-primary mb-4">
            ¡A Jugar!
          </h2>
          ...
          <div className="mb-6 p-4 bg-muted/50 rounded-[20px_8px_20px_8px/8px_20px_8px_20px] border-2 border-border">
              ...
              <div className="space-y-2 text-sm text-muted-foreground">
                <p className="flex justify-center gap-2">
                  <DoodleIcon
                    icon={UserSearch}
                    size={16}
                    className="stroke-[2.5] text-destructive"
                    uniqueId="classic-impostor"
                  />
                  <strong className="text-destructive">
                    {impostorIndices.length} Impostor
                    {impostorIndices.length > 1 ? "es" : ""}
                  </strong>
                  debe{impostorIndices.length > 1 ? "n" : ""} descubrir la
                  palabra
                </p>
              ...
              <div className="space-y-2 text-sm text-muted-foreground">
                <p className="flex justify-center gap-2">
                  <DoodleIcon
                    icon={Compass}
                    size={16}
                    className="stroke-[2.5] text-orange-500"
                    uniqueId="lost-player"
                  />
                  <strong className="text-orange-500">
                    Uno de los jugadores tiene una palabra diferente
                  </strong>
                </p>
                <p className="flex justify-center gap-2">
                  <DoodleIcon
              ...
              <div className="space-y-2 text-sm text-muted-foreground">
                <p className="flex justify-center gap-2">
                  <DoodleIcon
                    icon={Crown}
                    size={16}
                    className="stroke-[2.5] text-purple-500"
                    uniqueId="jester-player"
                  />
                  <strong className="text-purple-500">
                    Uno de los jugadores es el Bufón
                  </strong>
                </p>
                <p className="flex justify-center gap-2">
                  <DoodleIcon
              ...
              <div className="space-y-2 text-sm text-muted-foreground">
                {chaosAllImpostors ? (
                  <>
                    ...
                    <p>
                      • Todos deben descubrir la palabra sin ser descubiertos
                    </p>
                  ...
                  <>
                    <p className="flex justify-center gap-2">
                      <DoodleIcon
                        icon={Zap}
                        size={16}
                        className="stroke-[2.5] text-secondary"
                        uniqueId="chaos-all-different"
                      />
                    ...
                    <p className="flex justify-center gap-2">
                      <DoodleIcon
                        ...
                        size={16}
                        className="stroke-[2.5] text-secondary"
                        uniqueId="chaos-words"
                      />
                      <strong className="text-secondary">
                        ¡TODOS tienen palabras diferentes!
                      </strong>
                    </p>
                    <p>
                      • Nadie es impostor, pero cada uno tiene una palabra única
                    </p>
                  ...
                  <>
                    <p className="flex justify-center gap-2">
                      <DoodleIcon
                        icon={Zap}
                        size={16}
                        className="stroke-[2.5] text-green-500"
                        uniqueId="chaos-all-same"
                      />
                    ...
                    <p className="flex justify-center gap-2">
                      <DoodleIcon
                        ...
                        size={16}
                        className="stroke-[2.5] text-green-500"
                        uniqueId="chaos-same-word"
                      />
                      <strong className="text-green-500">
                        ¡NADIE es impostor, todos comparten la misma palabra!
                      </strong>
                    </p>
                  ...
                  <>
                    <p className="flex justify-center gap-2">
                      <DoodleIcon
                        ...
                        size={16}
                        className="stroke-[2.5] text-destructive"
                        uniqueId="chaos-normal-impostor"
                      />
                      <strong className="text-destructive">
                        {impostorIndices.length} Impostor
                        {impostorIndices.length > 1 ? "es" : ""}
                    ...
                    <p className="text-xs italic mt-2">
                      💡 El modo Locura puede activarse aleatoriamente
                    </p>
                  </>
                )}
              </div>
            ...
            {selectedGameMode === "random" && (
              <div className="space-y-2 text-sm text-muted-foreground">
                ...
                <p>
                  • Se ha elegido aleatoriamente uno de los modos disponibles
                </p>
                <p>• ¡Descubre las reglas mientras juegas!</p>
                <p className="text-xs italic mt-2">
                  🎲 El modo específico permanece secreto para todos
                </p>
              </div>
            )}
          </div>
          ...
          <p className="text-2xl text-foreground mb-8 flex items-center justify-center gap-2">
            Empieza:{" "}
            <span className="font-bold text-secondary inline-flex items-center gap-1 animate-[wiggle_0.5s_ease-in-out_infinite]">
